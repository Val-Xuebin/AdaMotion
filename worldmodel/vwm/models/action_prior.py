...
class ActionLatentPrior(nn.Module):
    family = "action_latent_prior"
    ...
    def __init__(
        ...
        self.past_proj = nn.Linear(vae_latent_dim, hidden_dim)
        self.net = nn.Sequential(
            nn.LayerNorm(hidden_dim * 2),
            nn.Linear(hidden_dim * 2, hidden_dim),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, future_len * action_dim),
        )
    ...
    def load_state_dict_without_clip(self, state_dict: Dict[str, torch.Tensor]) -> None:
        ...
        if unexpected_keys:
            raise ValueError(f"Unexpected keys when loading action prior: {unexpected_keys}")
        non_clip_missing = [key for key in missing_keys if not key.startswith("text_encoder.model.")]
        if non_clip_missing:
            raise ValueError(f"Missing non-CLIP keys when loading action prior: {non_clip_missing}")
    ...
    def forward(self, texts: list[str], past_latent: torch.Tensor) -> torch.Tensor:
        batch_size = past_latent.shape[0]
        ...
        past_hidden = self.past_proj(past_latent.reshape(batch_size, -1, past_latent.shape[-1]).mean(dim=1))
        action = self.net(torch.cat([text_hidden, past_hidden], dim=-1))
        return action.reshape(batch_size, self.future_len, self.action_dim)
